...
import { InlineChatWidget } from "./ResearchAssistant";
import {
  Loader2,
  ZoomIn,
  ZoomOut,
...
This is a paper written in Carbon. 
...
const PANEL_LAYOUT_KEY = "carbon-panel-layout";
const LATEX_CODE_KEY = "carbon-latex-code";
const DEFAULT_LAYOUT = { editor: 50, preview: 50 };
...
export default function LaTeXEditor() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("carbon-dark-mode");
  ...
  useEffect(() => {
    ...
    localStorage.setItem("carbon-dark-mode", String(isDarkMode));
  }, [isDarkMode]);
  ...
  const loadLayout = (): Record<string, number> => {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-zinc-50 dark:bg-zinc-950 overflow-hidden relative">
      {/* Toolbar */}
      <div className="h-12 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex items-center px-4 justify-between shrink-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`bg-black dark:bg-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-2 ${isCompiling ? "opacity-50" : "shadow-sm"}`}
          >
          ...
          <button
            ...
            className={`text-zinc-600 hover:text-zinc-900 border border-zinc-200 hover:border-zinc-300 hover:bg-zinc-50 px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-2 dark:text-zinc-400 dark:hover:text-zinc-100 dark:border-zinc-700 dark:hover:border-zinc-600 dark:hover:bg-zinc-800 ${!hasUnsavedChanges ? "opacity-50 cursor-default" : "shadow-sm"}`}
          >
            {hasUnsavedChanges ? (
              <Save size={12} />
            ) : (
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          ...
          >
            <div className="h-full w-full py-2 pl-2 pr-0 relative">
              <div className="h-full w-full bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden flex flex-col relative">
          ...
          <Separator className="w-2 bg-transparent hover:bg-zinc-200/50 dark:hover:bg-zinc-700/50 transition-colors flex items-center justify-center cursor-col-resize group z-10 rounded-sm">
            <div className="h-8 w-1 rounded-full bg-zinc-300 dark:bg-zinc-600 group-hover:bg-zinc-400 dark:group-hover:bg-zinc-500 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
              <GripVertical
                size={12}
          ...
          <Panel id="preview" defaultSize={50} minSize={5}>
            <div className="h-full w-full py-2 pl-0 pr-2">
              <div className="h-full w-full bg-zinc-100 dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden flex flex-col relative">
                <div className="h-full w-full transition-all duration-300 relative overflow-hidden flex flex-col">
                  ...
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white/90 dark:bg-zinc-900/90 backdrop-blur border border-zinc-200 dark:border-zinc-700 shadow-lg rounded-full px-4 py-2 flex items-center gap-4 transition-all z-20">
                    {/* Zoom Controls */}
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => setScale((s) => Math.max(0.5, s - 0.1))}
                        className="p-1.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
                        title="Zoom Out"
                      >
                        <ZoomOut size={16} />
                      </button>
                      <span className="text-xs font-medium w-12 text-center text-zinc-600 dark:text-zinc-400">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors ml-1"
                        title="Reset Zoom"
                      >
                        <RotateCcw size={14} />
                      </button>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-1.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                      >
                        <ChevronLeft size={16} />
                      </button>
